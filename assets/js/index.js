...
contenedor.innerHTML = indexEvent;
...
function buscando(categorias,ingresoTexto){
  ...
  if(categorias.length > 0 && ingresoTexto == ""){
    ...
    data.events.filter(event => categorias.includes(event.category)).filter(event => event.name.toLowerCase().includes(ingresoTexto)|| event.description.toLowerCase().includes(ingresoTexto)).forEach(event =>{
      Resultado += createCard(event)
    });
    if(Resultado == ""){
